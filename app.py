    ...
    try:
        ...
        for team_abbr in teams_today:
            try:
                ...
                # Only scan top players — limit per team to avoid too many API calls
                roster_items = list(roster.items())[:8]
                
                for player_name_raw, athlete_id in roster_items:
                    player_name = player_name_raw.title()
                    ...
                    if status != 200 or df.empty or len(df) < 5:
                        continue
                    
                    # Check active recently
                    ...
                    if days_out >= 10:
                        continue  # Skip likely injured/inactive
                    ...
                    season_avg = round(float(df[s_col].mean()), 1)
                    
                    # Skip low-usage players
                    min_avg = {'PTS': 8.0, 'TRB': 3.0, 'AST': 2.0, 'FG3M': 0.5,
                               'PRA': 15.0, 'PR': 10.0, 'PA': 10.0, 'RA': 5.0}.get(s_col, 5.0)
        ...
        if not heaters:
            return None, f"No NBA players found playing tonight."
        
        # Sort by season average descending
        df_out = pd.DataFrame(heaters).sort_values("Season Avg", ascending=False).head(20)
        return df_out, f"✅ NBA Radar Online: {len(df_out)} targets loaded for tonight."
    
    except Exception as e:
        return None, f"NBA Radar Error: {str(e)}"
@st.cache_data(ttl=3600)
...
def render_syndicate_board(league_key):
    ...
    st.markdown("""
    <style>
    div[data-testid="stRadio"] > div { display: flex; gap: 2px; background: #0f172a; border: 1px solid #334155; border-radius: 6px; padding: 3px; }
    div[data-testid="stRadio"] > div > label { flex: 1; text-align: center; padding: 5px 8px; border-radius: 4px; font-size: 11px; font-weight: 700; cursor: pointer; color: #94a3b8; margin: 0; }
    div[data-testid="stRadio"] > div > label:has(input:checked) { background: #1e293b; color: #f8fafc; }
    div[data-testid="stRadio"] > div > label input { display: none; }
    </style>
    """, unsafe_allow_html=True)

    # ── FULL TOP ROW IN STYLED BORDERED CONTAINER ──────────
    st.markdown("""
    <div style="background:#1e293b;border:1px solid #334155;border-radius:12px;
         padding:14px 16px;margin-bottom:10px;">
    """, unsafe_allow_html=True)

    tc1, tc2, tc3, tc4, tc5 = st.columns([1.0, 1.0, 0.8, 0.65, 1.6])
    ...
    with tc3:
        home_away = st.radio("Home/Away", ["🏠 Home", "✈️ Away"], horizontal=True, key=f"{lk}.home_away", label_visibility="collapsed")
        is_home_bool = home_away == "🏠 Home"
        is_home_current = 1 if is_home_bool else 0
        st.session_state[f"{lk}.is_home"] = is_home_bool
    ...
    with tc4:
        ...
        <div style="display:flex;align-items:center;gap:6px;padding:4px 0;">
            ...
            <span style="font-size:16px;font-weight:900;color:#00E5FF;">{opp}{teammate_html}</span>
        </div>
        """, unsafe_allow_html=True)

    with tc5:
        sp_c, fat_c = st.columns([0.6, 1])
        with sp_c:
            spread_input = st.number_input("Spread", min_value=-30.0, max_value=30.0, value=0.0, step=0.5, key=f"{lk}.spread", label_visibility="collapsed", format="%.1f")
        ...
        with fat_c:
            ...
            fat_sel = st.radio("Fatigue", fat_options, horizontal=True, key=f"{lk}.fat_radio", label_visibility="collapsed")
            rest = fat_map.get(fat_sel, fat_options[0])
    ...
    with add_c1:
        new_stat = st.selectbox("Stat", player_props, key=f"{lk}.new_stat", label_visibility="visible")
    with add_c2:
        new_line = st.number_input("Line", value=0.5, step=0.5, key=f"{lk}.new_line", label_visibility="visible", format="%.1f")
    with add_c3:
        new_odds = st.number_input("Odds", value=-110, step=5, key=f"{lk}.new_odds", label_visibility="visible")
    with add_c4:
        st.markdown("<div style='height:28px;'></div>", unsafe_allow_html=True)
        if st.button("+ Add Stat", key=f"{lk}.add_stat", type="primary", use_container_width=True):
            queued = st.session_state.get(f"{lk}.queued_stats", [])
            if not any(q['stat_type'] == new_stat for q in queued):
                queued.append({"stat_type": new_stat, "line": float(new_line), "odds": int(new_odds)})
                st.session_state[f"{lk}.queued_stats"] = queued
            st.rerun()

    queued_stats = st.session_state.get(f"{lk}.queued_stats", [])
    stat_results = st.session_state.get(f"{lk}.stat_results", [])
    results_map = {r['stat_type']: r for r in stat_results}

    if queued_stats:
        hc1, hc2, hc3, hc4, hc5 = st.columns([2.5, 0.8, 0.8, 1, 0.4])
        for h, t in zip([hc1, hc2, hc3, hc4, hc5], ["Stat", "Line", "Odds", "Projection", ""]):
            h.markdown(f"<div style='font-size:9px;font-weight:700;color:#94a3b8;letter-spacing:.7px;text-transform:uppercase;padding-bottom:4px;'>{t}</div>", unsafe_allow_html=True)

        for qi, qs in enumerate(queued_stats):
            result = results_map.get(qs['stat_type'])
            qc1, qc2, qc3, qc4, qc5 = st.columns([2.5, 0.8, 0.8, 1, 0.4])
            with qc1:
                st.markdown(f"<div style='padding:8px 0;font-size:13px;font-weight:600;color:#f8fafc;'>{qs['stat_type']}</div>", unsafe_allow_html=True)
            with qc2:
                nv = st.number_input("L", value=float(qs['line']), step=0.5, key=f"{lk}.ql_{qi}", label_visibility="collapsed")
                queued_stats[qi]['line'] = nv
            with qc3:
                no = st.number_input("O", value=int(qs['odds']), step=5, key=f"{lk}.qo_{qi}", label_visibility="collapsed")
                queued_stats[qi]['odds'] = no
            with qc4:
                if result:
                    v, p = result['vote'], result['proj']
                    vc = "#00c853" if v == "OVER" else ("#ff5252" if v == "UNDER" else "#94a3b8")
                    st.markdown(f"<div style='background:rgba(0,0,0,.2);border-radius:5px;padding:6px 8px;text-align:center;font-size:12px;font-weight:900;color:{vc};border:1px solid {vc}40;margin-top:2px;'>{p:.2f} {v}</div>", unsafe_allow_html=True)
                else:
                    st.markdown("<div style='background:#1e293b;border-radius:5px;padding:6px 8px;text-align:center;font-size:11px;color:#334155;border:1px solid #1e293b;margin-top:2px;'>—</div>", unsafe_allow_html=True)
            with qc5:
                if st.button("×", key=f"{lk}.del_{qi}", use_container_width=True):
                    st.session_state[f"{lk}.queued_stats"] = [q for i, q in enumerate(queued_stats) if i != qi]
                    st.session_state[f"{lk}.stat_results"] = [r for r in stat_results if r['stat_type'] != qs['stat_type']]
                    st.rerun()
        st.session_state[f"{lk}.queued_stats"] = queued_stats

    if player_name:
        st.session_state[f"{lk}.target_player"] = player_name
    target_player = st.session_state.get(f"{lk}.target_player")

    btn_c1, btn_c2 = st.columns([3, 1])
    with btn_c1:
        analyze_all = st.button("🚀 Analyze All Stats", type="primary", use_container_width=True, key=f"{lk}.analyze_all", disabled=not (target_player and queued_stats))
    with btn_c2:
        if st.button("🗑️ Clear All", use_container_width=True, key=f"{lk}.clear_all"):
            st.session_state[f"{lk}.queued_stats"] = []
    ...
    if analyze_all and target_player and queued_stats:
        ...
        if status_code == 429:
            st.error("🚨 Rate limited. Wait 60 seconds."); st.stop()
        elif df.empty:
            ...
            for qi, qs in enumerate(queued_stats):
                ...
                if league_key == "NBA":
                    if s_col == "A": s_col = "AST"
                    if s_col == "PRA" and 'PTS' in df_stat.columns: df_stat['PRA'] = df_stat['PTS'] + df_stat['TRB'] + df_stat['AST']
                    if s_col == "PR"  and 'PTS' in df_stat.columns: df_stat['PR']  = df_stat['PTS'] + df_stat['TRB']
                    if s_col == "PA"  and 'PTS' in df_stat.columns: df_stat['PA']  = df_stat['PTS'] + df_stat['AST']
                    if s_col == "RA"  and 'TRB' in df_stat.columns: df_stat['RA']  = df_stat['TRB'] + df_stat['AST']
                    if s_col in ["DD", "TD"]:
                        tens = (df_stat['PTS'] >= 10).astype(int) + (df_stat['TRB'] >= 10).astype(int) + (df_stat['AST'] >= 10).astype(int)
                        df_stat['DD'] = (tens >= 2).astype(int)
                        df_stat['TD'] = (tens >= 3).astype(int)
                if league_key == "MLB" and s_col == "HRR":
                    if 'H' in df_stat.columns and 'R' in df_stat.columns and 'RBI' in df_stat.columns:
                        df_stat['HRR'] = df_stat['H'] + df_stat['R'] + df_stat['RBI']
                    elif 'H' in df_stat.columns:
                        df_stat['HRR'] = df_stat['H']

                if s_col not in df_stat.columns:
                    continue

                df_hash = f"{len(df_stat)}_{str(df_stat['ValidDate'].iloc[-1])}_{df_stat[s_col].sum():.2f}"

                try:
                    df_ml, board, raw_consensus, raw_vote_from_board, c_color, mod_val, mod_desc, current_split_mod, split_text, split_desc, fatigue_val, fatigue_desc, archetype, raw_vote = run_ml_board(
                        df_stat, s_col, line, opp, league_key, rest, is_home_current, stat_type,
                        False, df_hash, ledger_hash, opp_pitcher_era, opp_pitcher_name
                    )
                except:
                    continue
                ...
                if st.session_state.get(f"{lk}.injury_boost", False):
                    ...
                    df_ml['AI_Proj'] = df_ml.get('AI_Proj', pd.Series([final_consensus] * len(df_ml))) * 1.08

                # Final vote
                dynamic_thresh = PASS_THRESHOLDS.get(s_col, 0.3)
                if s_col in COMBO_STATS: dynamic_thresh *= 1.30
                c_proj = final_consensus
                c_vote = "OVER" if c_proj >= line + dynamic_thresh else ("UNDER" if c_proj <= line - dynamic_thresh else "PASS")
                ...
                if edge_pct < 0 and c_vote != "PASS":
                    c_vote = "VETO"; c_color = "#ff0055"; rec_stake = 0.0
                ...
                new_results.append({
                    ...
                    "proj": c_proj, "vote": c_vote, "color": c_color,
                    "setup_score": setup_score, "win_prob": win_prob, "edge_pct": edge_pct,
                    "ev_dollars": ev_dollars, "rec_stake": rec_stake, "board": board,
                    "df_ml": df_ml, "df_l10": df_l10, "mod_desc": mod_desc, "mod_val": mod_val,
                    "archetype": archetype, "split_text": split_text, "split_desc": split_desc,
                    "current_split_mod": current_split_mod, "fatigue_val": fatigue_val,
            ...
            pb.empty()
            new_results.sort(key=lambda x: x['setup_score'], reverse=True)
            st.session_state[f"{lk}.stat_results"] = new_results
            st.rerun()

    # --- RESULTS DISPLAY ---
    stat_results = st.session_state.get(f"{lk}.stat_results", [])

    if stat_results and target_player:
        st.markdown("---")

        # MATCHUP INTEL
        first = stat_results[0]
        archetype = first.get('archetype', '')
        mod_val = first.get('mod_val', 1.0)
        split_text = first.get('split_text', 'Home')
        ...
        <div style="background:#1e293b;border:1px solid #334155;border-radius:8px;padding:12px;margin-bottom:12px;">
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;">
                <img src='{get_team_logo(league_key, player_team)}' width='22' style='vertical-align:middle;'>
                <span style="font-size:12px;font-weight:700;color:#00E5FF;letter-spacing:.5px;">MATCHUP INTEL</span>
                <span style="font-size:11px;color:#94a3b8;">— {target_player.split("(")[0].strip()} vs {opp}</span>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:8px;">
                ...
                <div style="background:#0f172a;border-radius:6px;padding:8px 10px;">
                    ...
                    <div style="font-size:12px;font-weight:700;color:{def_color};">{def_label}</div>
                </div>
                <div style="background:#0f172a;border-radius:6px;padding:8px 10px;">
                    <div style="font-size:9px;font-weight:700;color:#94a3b8;letter-spacing:.7px;text-transform:uppercase;margin-bottom:3px;">Venue / Split</div>
                    <div style="font-size:12px;font-weight:700;color:#f8fafc;">{split_text} {current_split_mod:.2f}×</div>
                </div>
                <div style="background:#0f172a;border-radius:6px;padding:8px 10px;">
        ...
        # RESULT CARDS
        selected_for_lock = set(st.session_state.get(f"{lk}.selected_for_lock", set()))
        rc1, rc2 = st.columns([3, 1])
        rc1.markdown("<div style='font-size:10px;font-weight:700;color:#94a3b8;letter-spacing:.8px;text-transform:uppercase;'>Board recommendations — ranked by setup score</div>", unsafe_allow_html=True)
        rc2.markdown(f"<div style='font-size:10px;font-weight:700;color:#00E5FF;text-align:right;'>{len(selected_for_lock)} selected</div>", unsafe_allow_html=True)

        for ri, result in enumerate(stat_results):
            ...
            rec_stake  = result['rec_stake']
            l10_hits   = result.get('l10_hits', 0)
            s_col      = result['s_col']
            is_top     = ri == 0 and vote not in ["PASS", "VETO", "SUPPRESSED"]
            is_checked = ri in selected_for_lock
            is_expanded = st.session_state.get(f"{lk}.expanded_{ri}", ri == 0)

            vote_color  = "#00c853" if vote == "OVER" else ("#ff5252" if vote == "UNDER" else ("#ff0055" if vote in ["VETO","SUPPRESSED"] else "#94a3b8"))
            score_color = "#FFD700" if setup_score >= 75 else ("#f59e0b" if setup_score >= 55 else "#94a3b8")
            score_label = "ELITE" if setup_score >= 75 else ("SOLID" if setup_score >= 55 else ("MARGINAL" if setup_score >= 35 else "WEAK"))
            border_style = f"border-left:4px solid {'#FFD700' if is_top else (vote_color if is_checked else '#334155')}"

            st.markdown(f"""
            <div style="background:#0f172a;{border_style};border:1px solid {'rgba(255,215,0,.4)' if is_top else ('#334155')};
                 border-radius:8px;padding:0;margin-bottom:2px;overflow:hidden;">
            </div>
            """, unsafe_allow_html=True)
            ...
            with rh_c0:
                ...
                if st.button(chk_label, key=f"{lk}.chk_{ri}", use_container_width=True):
                    ...
                    st.session_state[f"{lk}.selected_for_lock"] = selected_for_lock
                    st.rerun()

            with rh_c1:
                top_badge = " 🏆 TOP PICK" if is_top else ""
                agree = result.get('agree_count', 0)
                dots_html = "".join([
                    f"<div style='width:8px;height:8px;border-radius:50%;background:{'#00c853' if m['vote']=='OVER' else ('#ff5252' if m['vote']=='UNDER' else '#94a3b8')};display:inline-block;margin-right:2px;'></div>"
                    for m in result['board'][:5]
                ])
                st.markdown(f"""
                <div style='padding:4px 0;'>
                    <div style='font-size:13px;font-weight:700;color:{'#FFD700' if is_top else '#f8fafc'};'>{stat_type}{top_badge}</div>
                    <div style='font-size:10px;color:#94a3b8;margin-top:1px;'>Proj {proj:.2f} &nbsp;|&nbsp; Edge {result['edge_pct']:+.1f}% &nbsp;|&nbsp; Win Prob {result['win_prob']*100:.1f}%</div>
                    <div style='margin-top:3px;'>{dots_html}<span style='font-size:9px;color:#94a3b8;margin-left:3px;'>{agree}/5</span></div>
                </div>
                """, unsafe_allow_html=True)

            with rh_c2:
                st.markdown(f"<div style='text-align:center;padding-top:2px;'><div style='font-size:14px;font-weight:900;color:{score_color};'>{setup_score}</div><div style='font-size:9px;color:#94a3b8;'>{score_label}</div><div style='height:4px;background:#1e293b;border-radius:2px;overflow:hidden;margin-top:3px;'><div style='height:100%;width:{setup_score}%;background:{score_color};border-radius:2px;'></div></div></div>", unsafe_allow_html=True)

            with rh_c3:
                stake_str = f"${rec_stake:.2f}" if rec_stake > 0 else "$0"
            ...
            if is_expanded and result['board']:
                ...
                if st.button(f"🛎️ Consult CFO + COO on {stat_type}", key=f"{lk}.consult_{ri}", use_container_width=True):
                    with st.spinner("CFO and COO reviewing..."):
                        import json
                        context = json.dumps({
                            "player": target_player, "league": league_key,
                            "market": stat_type, "line": float(line_val), "odds": int(odds),
                            "opponent": opp, "fatigue": rest,
                            "ml_consensus_proj": round(float(proj), 2), "ml_vote": vote,
                            "win_probability_pct": round(float(result['win_prob'] * 100), 1),
                            "edge_pct": round(float(result['edge_pct']), 1),
                            "setup_score": setup_score, "archetype": archetype,
                            "board_consensus": f"{result.get('agree_count',0)}/5"
                        }, indent=2)
                        cfo_res, coo_res = consult_the_board(context)
                        cfo_res = cfo_res.replace("#", "")
                        coo_res = coo_res.replace("#", "")
                        st.markdown(f"""
                        <div style="background:#0f172a;border:1px solid #334155;border-radius:8px;padding:14px;margin-top:8px;">
                            <h4 style="color:#00E5FF;text-align:center;margin-bottom:12px;font-size:13px;">🏛️ THE SYNDICATE BOARDROOM</h4>
                        </div>
                        """, unsafe_allow_html=True)
                        brc1, brc2 = st.columns(2)
                        with brc1:
                            st.markdown(f"**👔 Claude (CFO)**\n\n> {cfo_res}")
                        with brc2:
                            st.markdown(f"**📋 Gemini (COO)**\n\n> {coo_res}")

                s_avg_v = result.get('s_avg', 0)
                l10_avg_v = result.get('l10_avg', 0)
                l5_avg_v = result.get('l5_avg_val', 0)
                trend_color = "#00c853" if l5_avg_v >= s_avg_v * 1.1 else ("#d50000" if l5_avg_v <= s_avg_v * 0.9 else "#fff")
                ac1, ac2, ac3 = st.columns(3)
                ac1.metric("Season Avg", s_avg_v)
                ac2.metric("L10 Avg", l10_avg_v)
                ac3.metric("L5 Avg", l5_avg_v)

                st.markdown(f"<div style='font-size:11px;color:{result['skynet_color']};margin:6px 0;'>{result['skynet_msg']}</div>", unsafe_allow_html=True)
                st.markdown("---")
        ...
        with lk_c1:
            if selected_for_lock:
                ...
                st.markdown(f"<div style='font-size:11px;color:#94a3b8;padding:8px 0;'>{len(selected_for_lock)} selected: {', '.join(names)}</div>", unsafe_allow_html=True)
            else:
