...
def run_ml_board(df, s_col, line, opp, league, rest, is_home_current, stat_type, ignore_blowout=False, df_hash="", ledger_hash=""):
    ...
    # Re-use the historical predictions generated by the Meta-Learner earlier
    # Apply the exact same dynamic weights to the historical chart plot
    ...
    board = [
        {"name": f"⏱️ MIN Maximizer ({w_poi*100:.0f}%)", "model": "Poisson Regressor", "proj": trend_proj, "vote": get_raw_vote(trend_proj), "color": get_final_vote(trend_proj)[1], "quote": f"MAE: {mae_poi:.2f} | Weighted by recent minutes."},
