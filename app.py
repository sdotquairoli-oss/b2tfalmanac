    ...
    if stat_results and target_player:
        ...
        for ri, result in enumerate(stat_results):
            ...
            with rh_c0:
                ...
                components.html(f"""
                <script>
                function styleChkBtn_{ri}() {{
                    const buttons = window.parent.document.querySelectorAll('button');
                    buttons.forEach(btn => {{
                        const p = btn.querySelector('p');
                        if (p && (p.textContent.trim() === '✓' || p.textContent.trim() === '·')) {{
                            const isChecked = p.textContent.trim() === '✓';
                            p.style.color = isChecked ? '#ff0055' : '#475569';
                ...
                styleChkBtn_{ri}();
                setTimeout(styleChkBtn_{ri}, 100);
                setTimeout(styleChkBtn_{ri}, 500);
                </script>
                """, height=0)
