        ...
        if r.status_code == 200:
            ...
            for g in games:
                ...
                if il or stat == "TBD":
                    ...
                    ds = f"Today - {stat.replace(' ET','').replace(' EST','').upper()}"
                ...
                for m in b.get('markets', []):
                    for o in m.get('outcomes', []):
                        if clean_name in o.get('description', '').lower():
                            if 'point' in o and 'price' in o: 
                                return float(o['point']), int(o['price']), f"Synced: {b.get('title')}", used, rem
                            elif 'price' in o: 
                                return None, int(o['price']), f"Synced Odds: {b.get('title')}", used, rem
...
def get_nba_stats(player_label):
    ...
    try:
        ...
        # 1. Search official NBA database for Player ID
        ...
        # 2. Pull the game log directly from stats.nba.com (Defaults to Current Season)
        log = playergamelog.PlayerGameLog(player_id=pid)
        df = log.get_data_frames()[0]
        
        if df.empty: return pd.DataFrame(), 404, []
        
        # 3. Format the data for our Machine Learning Models
        ...
        df = df.rename(columns={'REB': 'TRB'}) # ML expects TRB for Rebounds
        
        # Isolate exactly what the models need, sorted chronologically
...
def run_barn_burner():
    ...
    bad_defenses = ['SJS', 'ANA', 'CBJ', 'CHI', 'MTL', 'NYI']

    try:
        r = requests.get(f"{base_url}/schedule/now", timeout=5).json()
        if not r.get('gameWeek'): return None, "No games scheduled today."
        today_data = r['gameWeek'][0]
        date = today_data['date']
        games = today_data['games']
    except: return None, "Failed to connect to NHL Schedule API."

    matchups = [{'Home': g['homeTeam']['abbrev'], 'Away': g['awayTeam']['abbrev']} for g in games]
    results = []

    def get_roster(team_abbr):
        try:
            url = f"{base_url}/roster/{team_abbr}/current"
            r = requests.get(url, timeout=5).json()
            targets = []
            for cat in ['forwards', 'defensemen']:
                for p in r.get(cat, []):
                    targets.append({'Name': f"{p['firstName']['default']} {p['lastName']['default']}", 'ID': p['id']})
            return targets[:12]
        except: return []

    def analyze_bb_player(player_id, player_name, opponent):
        try:
            ...
            return {
                'Player': player_name, 'Opp': opponent, 'L5 Avg': round(l5_avg, 1),
                'Recent Log': str(sogs), 'Rating': rating, 'Diamond': is_diamond
            }
        except: return None

    for game in matchups:
        for side in [('Home', 'Away'), ('Away', 'Home')]:
            roster = get_roster(game[side[0]])
            for p in roster:
                data = analyze_bb_player(p['ID'], p['Name'], game[side[1]])
                if data: results.append(data)
                time.sleep(0.05)

    if not results: return None, f"Scan complete. No high-volume shooters found for {date}."
    df = pd.DataFrame(results)
    df['Rank'] = df.apply(lambda x: 1 if x['Diamond'] else (2 if "VOLUME" in x['Rating'] else 3), axis=1)
    df = df.sort_values(by=['Rank', 'L5 Avg'], ascending=[True, False]).head(15)
    return df.drop(columns=['Diamond', 'Rank']), f"✅ Barn Burner Scan Complete for {date}. Top SOG Targets Found!"

@st.cache_data(ttl=3600)
def run_mlb_heaters(target_stat="Hits"):
    base_url = "https://statsapi.mlb.com/api/v1"
    try:
        r = requests.get(f"{base_url}/schedule?sportId=1", timeout=5).json()
        dates = r.get('dates', [])
        if not dates: return None, "No MLB games scheduled today."
        games = dates[0].get('games', [])
        if not games: return None, "No MLB games scheduled today."
    except: return None, "Failed to connect to MLB Schedule API."

    teams_playing = []
    for g in games:
        teams_playing.append(g['teams']['away']['team']['id'])
        teams_playing.append(g['teams']['home']['team']['id'])

    results = []
    for team_id in teams_playing:
        try:
            roster_url = f"{base_url}/teams/{team_id}/roster"
            roster_data = requests.get(roster_url, timeout=5).json().get('roster', [])
            targets = []
            for p in roster_data:
                pos = p.get('position', {}).get('abbreviation', '')
                if target_stat == "Pitcher Strikeouts" and pos == "P": targets.append(p)
                elif target_stat != "Pitcher Strikeouts" and pos != "P": targets.append(p)
            
            for p in targets[:6]:
                player_id = p['person']['id']
                player_name = p['person']['fullName']
                log_url = f"{base_url}/people/{player_id}/stats?stats=gameLog&group={'pitching' if target_stat == 'Pitcher Strikeouts' else 'hitting'}"
                log_r = requests.get(log_url, timeout=5).json()
                splits = log_r.get('stats', [{}])[0].get('splits', [])
                if not splits: continue
                
                recent = splits[-5:] if len(splits) > 5 else splits
                stat_key = 'strikeOuts' if target_stat == "Pitcher Strikeouts" else ('homeRuns' if target_stat == "Home Runs" else 'hits')
                vals = [s.get('stat', {}).get(stat_key, 0) for s in recent]
                l5_avg = sum(vals) / len(vals)
                
                is_heater = False
                if target_stat == "Hits" and l5_avg >= 1.6: is_heater = True
                if target_stat == "Home Runs" and l5_avg >= 0.4: is_heater = True
                if target_stat == "Pitcher Strikeouts" and l5_avg >= 6.0: is_heater = True
                
                if is_heater:
                    results.append({
                        'Player': player_name, 'Stat': target_stat, 'L5 Avg': round(l5_avg, 1), 'L5 Log': str(vals)
                    })
                time.sleep(0.05)
        except: continue

    if not results: return None, f"Scan complete. No extreme {target_stat} heaters found for today's slate."
    df = pd.DataFrame(results)
    df = df.sort_values(by='L5 Avg', ascending=False).head(15)
    return df, f"🔥 Scan Complete! Top 15 {target_stat} Heaters found."
...
def estimate_alt_odds(orig_line, orig_odds, new_line, stat_type):
    if orig_line is None or orig_odds is None or orig_line == new_line: 
        return orig_odds
    
    p_orig = abs(orig_odds)/(abs(orig_odds)+100) if orig_odds < 0 else 100/(orig_odds+100)
    std_matrix = {
        "Points": 6.0, "Rebounds": 2.5, "Assists": 2.5, "Threes Made": 1.2, 
        "PRA (Pts+Reb+Ast)": 8.0, "Points + Rebounds": 7.0, "Points + Assists": 7.0, 
        "Rebounds + Assists": 3.5, "Minutes Played": 5.0,
        "Hits": 1.0, "Home Runs": 0.5, "Total Bases": 1.5, "Pitcher Strikeouts": 2.0, "Pitcher Earned Runs": 1.5,
        "Goals": 0.6, "Shots on Goal": 1.5
    }
...
def run_ml_board(df, s_col, line, opp, league, rest, is_home_current):
    df_ml = df.copy()
    ...
    if len(df_ml) < 5: 
        return df_ml, [], 0, "PASS", "#94a3b8", 1.0, "Not enough data", 1.0, "", "", 1.0, "", archetype
    ...
    consensus = (trend_proj + stat_proj + con_proj + base_proj + guru_proj) / 5
    ...
    df_ml['AI_Proj'] = (lr_hist + rf_hist + gb_hist + svm_hist + guru_hist) / 5
    ...
    con_exp = f"Flags a potential variance {dev_dir} from season norms, leaning {get_vote(con_proj)[0]}."
    base_exp = f"High-dimension mapping of minute/production correlation sets a baseline {get_vote(base_proj)[0]}."
    mod_short = mod_desc.split('(')[0].strip().lower()
    guru_exp = f"Factors in {mod_short} and {split_text.lower()} split to confirm the {get_vote(guru_proj)[0]}."

    board = [
        {"name": "⏱️ MIN Maximizer", "model": "Linear Regression", "proj": trend_proj, "vote": get_vote(trend_proj)[0], "color": get_vote(trend_proj)[1], "quote": min_exp},
        {"name": "📊 Statistician", "model": "Random Forest", "proj": stat_proj, "vote": get_vote(stat_proj)[0], "color": get_vote(stat_proj)[1], "quote": stat_exp},
        {"name": "🃏 Contrarian", "model": "Gradient Boosting", "proj": con_proj, "vote": get_vote(con_proj)[0], "color": get_vote(con_proj)[1], "quote": con_exp},
        {"name": "🛡️ Baseline", "model": "Support Vector Machine", "proj": base_proj, "vote": get_vote(base_proj)[0], "color": get_vote(base_proj)[1], "quote": base_exp},
        {"name": "🎯 Context Guru", "model": "Radar, Rest, Arena", "proj": guru_proj, "vote": get_vote(guru_proj)[0], "color": get_vote(guru_proj)[1], "quote": guru_exp}
    ]
    return df_ml, board, consensus, get_vote(consensus)[0], get_vote(consensus)[1], mod_val, mod_desc, current_split_mod, split_text, split_desc, fatigue_val, fatigue_desc, archetype

def render_syndicate_board(league_key):
    if league_key == "NBA":
        sport_path = "basketball_nba"
        teams = NBA_TEAMS
    elif league_key == "MLB":
        sport_path = "baseball_mlb"
        teams = MLB_TEAMS
    else:
        sport_path = "icehockey_nhl"
        teams = NHL_TEAMS
    
    top_c1, top_c2, _ = st.columns([1, 1, 2])
    with top_c1:
        sync = st.toggle("📡 Auto-Sync Vegas Odds", value=False, key=f"sy_{league_key}")
    with top_c2:
    ...
    with st.container():
        ...
        with c2: 
            if league_key == "NBA":
                opts = ["Points", "Rebounds", "Assists", "Threes Made", "PRA (Pts+Reb+Ast)", "Points + Rebounds", "Points + Assists", "Rebounds + Assists", "Minutes Played"]
            elif league_key == "MLB":
                opts = ["Hits", "Home Runs", "Total Bases", "Pitcher Strikeouts", "Pitcher Earned Runs"]
            else:
                opts = ["Points", "Goals", "Assists", "Shots on Goal"]
        ...
        with c3: 
            ...
            if sync and f_line is not None and f_odds is not None:
                st.session_state[f"odds_{league_key}"] = estimate_alt_odds(float(f_line), int(f_odds), line, stat_type)
            elif f"odds_{league_key}" not in st.session_state:
                st.session_state[f"odds_{league_key}"] = -110
    ...
    with btn_c1:
        analyze_pressed = st.button(f"🚀 Analyze {league_key} Player", type="primary", use_container_width=True, key=f"btn_analyze_{league_key}")
    ...
    if target_player:
        ...
        if status_code == 429: 
            ...
            s_map = {
                ...
                "Minutes Played": "MINS", "Threes Made": "FG3M",
                "Points + Rebounds": "PR", "Points + Assists": "PA", "Rebounds + Assists": "RA",
                "Hits": "H", "Home Runs": "HR", "Total Bases": "TB", "Pitcher Strikeouts": "K", "Pitcher Earned Runs": "ER"
            }
            ...
            df_with_ml, board, c_proj, c_vote, c_color, mod_val, mod_desc, current_split_mod, split_text, split_desc, fatigue_val, fatigue_desc, archetype = run_ml_board(df, s_col, line, opp, league_key, rest, is_home_current)
            ...
            if len(board) == 0:
                ...
                if c_vote == "OVER":
                    ...
                    ai_summary_short = f"Projected to stay under {line} with a {win_prob*100:.1f}% probability."
                else:
                    ai_summary_short = f"Projection too close to {line} to recommend."
                    
                if league_key == "NBA" and "Exploit" in mod_desc:
                    ai_summary_short += f"<br><span style='color:#FFD700; font-weight:bold;'>🚨 Archetype Exploit vs {opp}</span>"
                elif league_key == "NBA" and "Fade" in mod_desc:
                    ai_summary_short += f"<br><span style='color:#ff0055; font-weight:bold;'>🛑 Archetype Fade vs {opp}</span>"
                ...
                with chart_col:
                    ...
                    ai_line = alt.Chart(df_l10).mark_line(color='#00E5FF', strokeWidth=3, point=alt.OverlayMarkDef(color='#00E5FF', size=60)).encode(
                        x=alt.X('Matchup_Label', sort=None), y=alt.Y('AI_Proj')
                    )
                    ...
                    text = bars.mark_text(
                        ...
                        text=alt.Text(s_col, format='.0f'),
                        color=alt.value('#ffffff')
                    )
                    
                    final_chart = (bars + vegas_rule + ai_line + text).configure(background='transparent').configure_axis(
                        gridColor='#334155', domainColor='#334155', tickColor='#334155', labelColor='#94a3b8', titleColor='#f8fafc'
                    ).configure_view(strokeWidth=0)
                    
                    st.altair_chart(final_chart, use_container_width=True)
                    st.caption("🟡 Dashed Yellow Line: Vegas Line &nbsp; | &nbsp; 🔵 Solid Cyan Line: AI Projection")
                ...
                with side_col:
                    with st.expander("📊 Matchup Intel (Team Stats)", expanded=True):
                        ...
                        if not df_opp.empty:
                            ...
                            st.markdown("<div style='font-size:14px; color:#94a3b8;'>No recent data vs this team.</div>", unsafe_allow_html=True)
            ...
            st.warning(f"🚨 **Warning:** No stats available for {target_player}. They may be a rookie or not logged yet.")
...
if os.path.exists(video_path):
    ...
    .b2tf-video-bg {{
      ...
      opacity: 0.6;
      object-fit: cover;
      animation: fade-out-video 2.3s ease-out 4.6s forwards; 
    }}
    ...
    .b2tf-content {{
      ...
      z-index: 1;
      opacity: 0;
      animation: fade-in-text 4.7s ease-out 4.5s forwards; 
    }}
    ...
    h1 {{
      ...
      font-weight: 900; 
      margin: 0; 
      text-shadow: 0 0 10px #ff6600, 0 0 20px #ff0000, 0 0 30px #ff0000; 
      letter-spacing: 2px;
    }}
    ...
    <body>
        <div class="b2tf-header-container">
            <video id="delorean-vid" class="b2tf-video-bg" autoplay muted playsinline>
                <source src="data:video/mp4;base64,{video_base64}" type="video/mp4">
            </video>
            <div class="b2tf-content">
                <h1>B2TF ALMANAC</h1>
                <div class="subtitle">ROADS? WHERE WE'RE GOING, WE DON'T NEED ROADS.</div>
            </div>
        </div>

        <script>
            var video = document.getElementById('delorean-vid');
    ...
    st.error("⚠️ The system cannot find the video file. Please ensure the video is in the exact same folder as `app.py` and is named exactly `delorean.mp4`.")


# --- SYSTEM DIAGNOSTICS & FUEL GAUGE ---
with st.expander("⛽ System Diagnostics & API Fuel Gauge", expanded=False):
    ...
    with diag_c2:
        ...
        if ODDS_API_KEY:
            ...
            with f_col2:
                if st.button("🔄 Check", key="chk_quota"):
                    check_api_quota()
            with f_col1:
                if 'api_used' in st.session_state and 'api_remaining' in st.session_state:
                    used = int(st.session_state['api_used'])
                    rem = int(st.session_state['api_remaining'])
                    total = used + rem
                    fuel_pct = rem / total if total > 0 else 0.0
                    
                    color = "#00E676" if fuel_pct > 0.3 else ("#FFD700" if fuel_pct > 0.1 else "#ff0055")
                    st.markdown(f"""
                    <div style="background-color: #1e293b; border-radius: 5px; width: 100%; height: 20px; border: 1px solid #334155; margin-top: 5px;">
                        <div style="background-color: {color}; width: {fuel_pct*100}%; height: 100%; border-radius: 4px; transition: width 0.5s;"></div>
                    </div>
                    <div style="font-size: 12px; color: #94a3b8; margin-top: 5px; text-align: right;">
                        {rem} / {total} Requests Remaining
                    </div>
                    """, unsafe_allow_html=True)
                else:
                    st.caption("Sync a bet or hit 'Check' to load data.")
        else:
...
with tab_nba:
    ...
    with t_col2:
        if st.button("🔄 Refresh", key="ref_nba"):
            st.rerun()
            
    with st.spinner("Loading today's matchups..."):
        schedule_data, msg = get_nba_schedule()
        
    if schedule_data:
        render_scoreboard(schedule_data)
    else:
        st.info(msg)
...
with tab_nhl:
    ...
    with t_col1:
        st.markdown("### 📅 Today's Slate")
    with t_col2:
        if st.button("🔄 Refresh", key="ref_nhl"):
            st.rerun()
            
    with st.spinner("Loading today's matchups..."):
        schedule_data, msg = get_nhl_schedule()
...
with tab_mlb:
    ...
    with t_col1:
        st.markdown("### 📅 Today's Slate")
    with t_col2:
        if st.button("🔄 Refresh", key="ref_mlb"):
            st.rerun()
            
    with st.spinner("Loading today's matchups..."):
        schedule_data, msg = get_mlb_schedule()
    ...
    with st.expander("⚾ Launch MLB Heaters Scanner", expanded=False):
        ...
        with heat_col1:
            target_stat = st.selectbox("Select Target Stat", ["Hits", "Home Runs", "Pitcher Strikeouts"])
    ...
    if roi_mode == "💵 Wallet Manager":
        ...
        with bw_c2:
            ...
            <div style="background-color: #1e293b; border: 1px solid #334155; border-radius: 8px; padding: 20px; text-align: center; margin-top: 28px;">
                ...
                <div style="color: #00E676; font-size: 36px; font-weight: 900; margin: 10px 0px;">${max(total_liquid, 0.0):.2f}</div>
                <div style="display: flex; justify-content: space-between; font-size: 12px; border-top: 1px dashed #334155; padding-top: 12px; margin-top: 15px;">
                    ...
                    <span style="color: #94a3b8;">Net Casino: <span style="color: {cas_color};">{tot_cas:+.2f}</span></span>
                    <span style="color: #94a3b8;">Sports Profit: <span style="color: {spo_color};">${tot_sports:+.2f}</span></span>
                </div>
            </div>
            """, unsafe_allow_html=True)
        ...
        if book_balances:
            ...
            for i, (book, bal) in enumerate(book_balances.items()):
                col = port_cols[i % 4]
                color = "#00E676" if bal >= 0 else "#ff0055"
        ...
        with roi_col2:
            if st.button("🤖 Auto-Grade Pending", type="primary", use_container_width=True):
                with st.spinner("Checking official APIs for final game stats..."):
                    _, grade_msg = auto_grade_ledger()
        ...
        if not ledger_df.empty:
            ...
            for i, row in reversed_df.iterrows():
                ...
                html = f"""
                <div style="background-color: #0f172a; border-radius: 8px; border: 1px solid #334155; border-left: 6px solid {status_color}; padding: 12px; margin-bottom: 5px;">
                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                        <span style="font-size: 12px; color: #94a3b8; font-weight: bold; text-transform: uppercase;">{row['League']} • {row['Date']}</span>
                        <span style="font-size: 14px; color: #fff; font-weight: bold;">{o:+d}</span>
                    </div>
                    <div style="font-size: 16px; font-weight: 900; color: #00E5FF;">{row['Player']}</div>
                    <div style="font-size: 14px; font-weight: bold; color: #f8fafc; margin-top: 2px;">{row['Stat']} <span style="color: #00E676;">{row['Vote']} {row['Line']}</span></div>
                    <div style="margin-top: 10px; border-top: 1px dashed #334155; padding-top: 8px; display: flex; justify-content: space-between;">
                        <span style="font-size: 12px; color: #94a3b8;">Risk: $100.00 &nbsp;|&nbsp; <span style="color: #00E5FF; font-weight: bold;">AI Prob: {win_prob_val:.1f}%</span></span>
                        <span style="font-size: 12px; font-weight: bold; color: {status_color};">Payout: ${total_payout:.2f}</span>
                    </div>
                </div>
                """
                ...
                with sc1:
                    ...
                    if row['Result'] == 'Loss':
                        ...
                        if st.session_state.get(f"autopsy_{orig_idx}"):
                            st.markdown(f"""
                            <div style="background-color: rgba(255, 0, 85, 0.1); border-left: 3px solid #ff0055; padding: 10px; margin-top: -5px; margin-bottom: 10px; font-size: 13px; color: #f8fafc;">
                                {st.session_state[f"autopsy_{orig_idx}"]}
                            </div>
                            """, unsafe_allow_html=True)
        ...
        for p in selected_picks:
            o = pick_odds_map[p]
            if o > 0: 
                calc_dec *= (o / 100.0) + 1.0
            else: 
                calc_dec *= (100.0 / abs(o)) + 1.0
            c_prob *= pick_prob_map.get(p, 0.55)
                
        if selected_picks:
            if calc_dec >= 2.0: 
                true_american = int(round((calc_dec - 1.0) * 100))
            else: 
                true_american = int(round(-100.0 / (calc_dec - 1.0)))
        ...
        with st.expander("📘 Open Bankroll Advisor (Calculate Bet Size)", expanded=False):
            ...
            if "Micro" in strat_toggle:
                ...
                st.markdown(f"""
                <div style="background-color: #0f172a; padding: 15px; border-radius: 8px; border-left: 4px solid #ff0055; margin-top: 10px;">
                    <div style="display: flex; justify-content: space-around; text-align: center;">
                        <div><div style="font-size: 12px; color: #94a3b8;">Standard Single Wager</div><div style="font-size: 24px; font-weight: 900; color: #ff0055;">${s_rec:.2f}</div></div>
                        <div><div style="font-size: 12px; color: #94a3b8;">Standard Parlay Risk</div><div style="font-size: 24px; font-weight: 900; color: #00E5FF;">${p_rec:.2f}</div></div>
                    </div>
                </div>
                """, unsafe_allow_html=True)
            else:
                ...
                st.markdown(f"""
                <div style="background-color: #0f172a; padding: 15px; border-radius: 8px; border-left: 4px solid #00E676; margin-top: 10px; text-align: center;">
                    <div style="font-size: 12px; color: #94a3b8;">Recommended Kelly Stake (Half-Kelly)</div>
                    <div style="font-size: 24px; font-weight: 900; color: #00E676;">${s_rec:.2f}</div>
                </div>
                """, unsafe_allow_html=True)
        
        p_col1, p_col2, p_col3, p_col4 = st.columns([2.5, 1, 1, 1.5])
        with p_col1:
            p_desc = st.text_area("Bet Description", value=default_desc, height=68)
        with p_col2:
        ...
        if p_odds != 0:
            if p_odds > 0:
                proj_profit = p_risk * (p_odds / 100)
            else:
                proj_profit = p_risk / (abs(p_odds) / 100)
        else:
            proj_profit = 0.0
        ...
        if st.button("➕ Add Bet to Tracker", type="primary"):
            if p_desc:
                ...
                st.error("Please enter a description or link your picks above.")
        ...
        if not parlay_df.empty:
            ...
            for i, row in reversed_p_df.iterrows():
                ...
                <div style="background-color: #0f172a; border-radius: 8px; border: 1px solid #334155; border-left: 6px solid {status_color}; padding: 12px; margin-bottom: 5px;">
                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                        <span style="font-size: 12px; color: #94a3b8; font-weight: bold; letter-spacing: 1px;">{book_lbl} • {row['Date']}</span>
                        <span style="font-size: 14px; color: #fff; font-weight: bold;">{o:+d}</span>
                    </div>
                    <div style="font-size: 13px; color: #f8fafc; margin-bottom: 10px; line-height: 1.5;">
                        {legs_html}
                    </div>
                    <div style="margin-top: 10px; border-top: 1px dashed #334155; padding-top: 8px; display: flex; justify-content: space-between;">
                        <span style="font-size: 12px; color: #94a3b8;">{risk_txt}</span>
                        <span style="font-size: 12px; font-weight: bold; color: {status_color};">Payout: ${total_payout:.2f}</span>
                    </div>
                </div>
                """
