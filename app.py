...
def auto_grade_ledger():
    ...
    for idx, r in df[df['Result'] == 'Pending'].iterrows():
        # 🚨 THE MONEYLINE BYPASS: Skips AI Grader for Team Markets
...
def render_league_scanners(league_name):
    ...
    with st.expander(f"📡 Launch {league_name} Skynet Radar", expanded=False):
        ...
        if league_name == "NBA":
            ...
            with c1: 
                scan_stat = st.selectbox("🎯 Target Stat", ["Points", "Rebounds", "Assists", "Threes Made", "PRA (Pts+Reb+Ast)", "Points + Rebounds", "Points + Assists", "Rebounds + Assists"], key=f"{lk}.scan_stat")
            with c2: 
...
def render_syndicate_board(league_key):
    ...
    with st.container():
        ...
        with c1: 
            ...
            player_name = None
            if search_query:
                # Allows manual bypass for team names directly typed in
        ...
        with c2: 
            # 🚨 MONEYLINE ADDED TO DROPDOWN HERE
        ...
        with c3: 
            ...
            if stat_type in ["Moneyline"]: start_line = 0.0 # ML has no line usually
    ...
    if target_player:
        # 🚨 THE MONEYLINE BYPASS: Prevents ML Engine from crashing on Team Bets
        if stat_type in ["Moneyline", "Spread", "Total (O/U)"]:
            ...
            if st.button(f"🔒 Lock {stat_type} Pick into ROI Ledger", use_container_width=True, type="primary", key=f"{lk}.lock_team"):
                # Save to Ledger with dummy AI data so it renders beautifully in ROI tab
                ...
                if len(board) == 0: st.warning(f"⚠️ **Insufficient Data:** {target_player} has played fewer than 5 games this season.")
                    ...
                    with side_col:
                        with st.expander("📊 Matchup Intel (Team Stats)", expanded=True):
                            # 🟢 Notice how these are pushed one 'Tab' to the right!
...
with t_roi:
    ...
    if not ledger_df.empty:
        ...
        for i, row in ledger_df.reset_index().iloc[::-1].iterrows():
            ...
            # 🚨 MONEYLINE UI FIX: Ensures Team bets look clean on the ROI Tracker
...
with t_wallet:
    ...
    # 🚨 THE MANUAL LOGGER: A dedicated UI piece right below the pie chart
    st.markdown("---")
